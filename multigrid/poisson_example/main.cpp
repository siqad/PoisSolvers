/*
    ...
    Supports only 2D Poisson's equation

MODIFICATIONS:
  -Added cout statements to visualise program flow
  -Make sure to enable --layout=tagged when installing Boost.
    The library that is looked for has the extra -mt tag on it (for multithreading),
    and the tag appears only on older installs of Boost or when --layout=tagged is set.
*/
