...
void create_electrodes(const int[], const double[], const double[], double*, std::pair<int,double>*, double*, int);
void workingfunc(int);

int main(void){
  for(int i = 1; i < 2; i++){
    workingfunc(i); //i is the current time step, used in create_electrodes.
  }
}

void workingfunc(int step){
  ...
  const int ns[3] = {110, 110, 110}; //x, y, z gridpoint numbers
  ...
  create_electrodes(ns, ds, Ls, RHS, electrodemap, chi, step);
...
void create_electrodes(const int ns[3], const double ds[3], const double Ls[3], double* RHS, std::pair<int,double> *electrodemap, double* chi, int step){
